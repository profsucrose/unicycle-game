...
// TODO: Migrate to actual types/cbs
...
namespace Events {
    ...
    export namespace Client {
        export interface PlayerMove extends Events.Data.PlayerPose {}
        export interface PlayerInit extends Events.Data.PlayerPose {}
    }
...
class Client {
    ...
    join(pose: PlayerPose) {
        this.socket.emit(eventTypes.CLIENT_PLAYER_INIT, pose)
    }

    updatePose(pose: PlayerPose) {
        this.socket.emit(eventTypes.CLIENT_PLAYER_MOVE, pose)
    }
}
...
;(async () => {
    // TODO: Types
    const playerInit: any = await new Promise((resolve, reject) => {
        client.socket.on(eventTypes.SERVER_PLAYER_INIT, (payload) => {
            resolve(payload)
        })
    })

    const unicycles: {[key: string]: Unicycle} = {}

    // TODO: Move to class
    const addUnicyclePlayer = (
        uuid: string,
        name: string,
        x: number,
        y: number,
        ...
        const playerUnicycle = new Unicycle(scene, name)
        playerUnicycle.worldMesh.position.set(x, y, z)
        playerUnicycle.worldMeshYaw = yaw
        // playerUnicycle.worldMeshRoll = roll
        unicycles[uuid] = playerUnicycle
    }

    const player = playerInit.player
    const currentPlayers = playerInit.currentPlayers
    ...
    const unicycle = new Unicycle(scene, player.name)
    unicycle.setPosition(new THREE.Vector3((player.x, player.y, player.z)))
    unicycle.worldMeshYaw = player.yaw

    const cameraController = new UnicycleCameraController(camera, unicycle)

    client.socket.on(eventTypes.SERVER_PLAYER_JOIN, payload => {
    ...
    client.socket.on(eventTypes.SERVER_PLAYER_MOVE, payload => {
        // TODO: Error handling
        console.log('Player moved', payload, unicycles)
        if (!(payload.uuid in unicycles)) return
        const u = unicycles[payload.uuid] as Unicycle
        u.worldMesh.position.set(payload.x, payload.y, payload.z)
        u.worldMeshRoll = payload.roll
        u.worldMeshYaw = payload.yaw
    })

    client.socket.on(eventTypes.SERVER_PLAYER_LEAVE, payload => {
        const uuid = payload.uuid
        // TODO: Error handling
        const u = unicycles[uuid] as Unicycle
        u.destroy()
        delete unicycles[uuid]
    })
    ...
    function loop() {
        ...
        client.socket.emit(eventTypes.CLIENT_PLAYER_MOVE, {
            x: unicycle.worldMesh.position.x,
            y: unicycle.worldMesh.position.y,
            z: unicycle.worldMesh.position.z,
            yaw: unicycle.worldMeshYaw,
            roll: unicycle.worldMeshRoll,
        })

        Object.values(unicycles).forEach(u => u.update())
